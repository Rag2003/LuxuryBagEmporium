...
function App() {
  ...
  const item = {}; // define the item object
...
const addToCartWithMessage = (product) => {
  addToCart(product); // Call the original addToCart function
  setShowAddedMessage(true); // Show the message
  setTimeout(() => setShowAddedMessage(false), 2000); // Hide the message after 2 seconds
};
  ...
  const addToCart = (product) => {
    ...
    if (index >= 0) {
      ...
      const updatedCart = [...cart, {       ...product,       quantity: 1,       title: product.title,       price: product.price     }];
      setCart(updatedCart);
    }
  };

  const msgToCart = (product) => {
  ...
  const steps = [
    {
      id: "Greet",
      message: "Hello, Welcome to LuxuryBagEmporium",
      trigger: "Ask Name",
    },
    {
      id: "Ask Name",
    ...
    {
      id: "Hi there",
      message: "Hi {previousValue}, what seems to be the problem?",
      trigger: "Problem",
    },
    {
      id: "Problem",
    ...
    },
    {
      id: "Quality",
      message:
        "The quality of our products is the best in LuxuryBagEmporium. There is nothing to worry about. How else may we assist you?",
      trigger: "Problem",
    },
    {
      id: "Delivery",
      message: "The delivery time is 2-3 days. How else may we assist you?",
      trigger: "Problem",
    },
    {
      id: "Contact",
      message:
        "Our contact number is 515-715-7755. How else may we assist you?",
      trigger: "Problem",
    },
  ];
...
const showAllItems = product.filter((el) => el.title.toLowerCase().includes(searchTerm.toLowerCase()))
  .map((el) => (
    <div
      key={el._id}
      style={{
...
<div style={{ backgroundColor: '#f8f9fa', padding: '10px' }}>
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: '800px', margin: '0 auto' }}>
    <button class={view === 'HOME' ? 'active' : ''} onClick={() => setView('HOME')} style={{ color: '#343a40', textDecoration: 'none', fontWeight: 600, fontSize: '18px', marginRight: '10px', cursor: 'pointer', transition: 'all 0.3s ease-in-out' }} 
       onMouseOver={(e) => {
    ...
    <button class={view === 'POST' ? 'active' : ''} onClick={() => setView('POST')} style={{ color: '#343a40', textDecoration: 'none', fontWeight: 600, fontSize: '18px', marginRight: '10px', cursor: 'pointer', transition: 'all 0.3s ease-in-out' }}
       onMouseOver={(e) => {
         e.target.style.transform = 'scale(1.1)';
         e.target.style.color = '#007bff';
       }}
       ...
       <button class={view === 'UPDATE' ? 'active' : ''} onClick={() => setView('UPDATE')} style={{ color: '#343a40', textDecoration: 'none', fontWeight: 600, fontSize: '18px', marginRight: '10px', cursor: 'pointer', transition: 'all 0.3s ease-in-out' }}
       onMouseOver={(e) => {
         e.target.style.transform = 'scale(1.1)';
         e.target.style.color = '#007bff';
       }}
    ...
    <button class={view === 'DELETE' ? 'active' : ''} onClick={() => setView('DELETE')} style={{ color: '#343a40', textDecoration: 'none', fontWeight: 600, fontSize: '18px', marginRight: '10px', cursor: 'pointer', transition: 'all 0.3s ease-in-out' }}
       onMouseOver={(e) => {
         e.target.style.transform = 'scale(1.1)';
         e.target.style.color = '#007bff';
       }}
    ...
    <button class={view === 'ABOUT' ? 'active' : ''} onClick={() => setView('ABOUT')} style={{ color: '#343a40', textDecoration: 'none', fontWeight: 600, fontSize: '18px', marginRight: '10px', cursor: 'pointer', transition: 'all 0.3s ease-in-out' }}
       onMouseOver={(e) => {
         e.target.style.transform = 'scale(1.1)';
         e.target.style.color = '#007bff';
       }}
       ...
       <button class={view === 'HELP' ? 'active' : ''} onClick={() => setView('HELP')} style={{ color: '#343a40', textDecoration: 'none', fontWeight: 600, fontSize: '18px', marginRight: '10px', cursor: 'pointer', transition: 'all 0.3s ease-in-out' }} 
       onMouseOver={(e) => {
         e.target.style.transform = 'scale(1.1)';
         e.target.style.color = '#007bff';
       }}
       ...
       <button class={view === 'CART' ? 'active' : ''} onClick={() => setView('CART')} style={{ backgroundColor: 'transparent', border: 'none', cursor: 'pointer', marginRight: '10px' }}>
 ...
 <div id="read-view" style={{textAlign: 'center', backgroundColor: 'linear-gradient(#FFA07A, #8ED6FF)'}}>
  ...
  <div style={{display: 'flex', flexWrap: 'wrap', justifyContent: 'center'}}>
    <div style={{margin: '1rem', padding: '1rem', backgroundColor: '#f2f2f2', borderRadius: '5px', minWidth: '300px', flex: 1}}>
      ...
      <button style={{padding: '0.75rem 1.25rem', backgroundColor: '#4CAF50', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', marginBottom: '1rem'}} onClick={() => toggleDescription('raghuram')}>
...
{view === 'CART' && (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
    ...
    {cart.map((item, index) => (
      ...
      >
        <img src={item.image} alt={item.title} style={{ maxWidth: '100px', marginRight: '20px', borderRadius: '10px' }} />
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <p style={{ marginBottom: '5px', fontWeight: 'bold', fontSize: '1.2rem' }}>{item.title}</p>
          <p style={{ marginBottom: '5px', color: '#888' }}>${item.price}</p>
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
            <label htmlFor={`quantity-${index}`} style={{ marginRight: '10px', fontWeight: 'bold' }}>Quantity:</label>
            <input 
              type="number" 
              id={`quantity-${index}`} 
              min="1" 
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '20px' }}>
      <p style={{ marginBottom: '5px', fontWeight: 'bold' }}>Total:</p>
      <p style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>${cart.reduce((total, item) => total + item.price * item.quantity, 0).toFixed(2)}</p>
    </div>
    <button onClick={() => setView('CHECKOUT')} disabled={cart.length === 0} style={{ 
      marginTop: '20px',
...
{view === 'CHECKOUT' && (
  ...
  <input type="text" id="name" name="name" placeholder="Raghuram Guddati" />
  ...
  <input type="text" id="cvv" name="cvv" placeholder="465" />

  <label htmlFor="address">Billing Address</label>
  <input type="text" id="address" name="address" placeholder="123 Main St" />

  <label htmlFor="zipcode">Zip Code</label>
  <input type="text" id="zipcode" name="zipcode" placeholder="50014" />

  <button onClick={() => setView('CART')} style={{ marginRight: '200px' }}>Back</button>

  <button id="submit-button" onClick={() => setView('THANK')}>Submit</button>
...
{view === 'THANK' && (
  <div className="thank-you-container" style={{ backgroundColor: '#b580ff5e' }}>
    <h1>Order Confirmed!</h1>
    <div className="thank-you-animation">
      <span>Thank you for shopping with us!</span>
      <span>For any questions please go to the help section to clarify.</span>
    </div>
    <button onClick={() => setView('HELP')} className="help-button">help</button>
  </div>
)}
